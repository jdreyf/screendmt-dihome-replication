...
dta <- dplyr::bind_rows(data.frame(Study = "Study 1", BMI = rep(nm.pheno$BMI, times=2), dihome = rep(dihomes, each=nrow(nm.pheno)), 
  ...
  dplyr::mutate(value = case_when(value > 5 ~ sort(value, decreasing = TRUE)[3], .default = value))
...
We plot the diHOMEs against BMI in both studies and for improved visualization we windsorize (or trim) 9,10-diHOME values 
